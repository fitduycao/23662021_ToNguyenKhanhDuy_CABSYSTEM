...
## Project: CAB System – Nền Tảng Đặt Xe Trực Tuyến
...
Tài liệu Đặc tả Yêu cầu Phần mềm (SRS) này xác định các yêu cầu chức năng, phi chức năng, quy tắc nghiệp vụ và giao diện tích hợp cho hệ thống **CAB System** của Công ty ABC. Tài liệu là cơ sở pháp lý và kỹ thuật cho đội ngũ phát triển, kiểm thử, quản trị dự án và ban lãnh đạo công ty nghiệm thu sản phẩm.

### 1.2 Phạm vi dự án (Project Scope)
- **Tên dự án:** CAB System (Nền tảng đặt xe trực tuyến).
- **Mục tiêu:** Thay thế quy trình điều phối thủ công và ứng dụng đơn giản hiện tại bằng nền tảng số hóa toàn diện: tự động phân bổ tài xế, theo dõi lộ trình thời gian thực, quản trị giao dịch tập trung và tối ưu hóa vận hành.
- **Phạm vi triển khai (7 tuần):**
  - Khách hàng (Customer Web/App): Đăng ký/đăng nhập, tạo cuốc, theo dõi xe, thanh toán, đánh giá.
  - Tài xế (Driver App): Nhận thông báo cuốc, cập nhật trạng thái làm việc, cập nhật tiến trình chuyến đi, định vị GPS.
  - Bộ phận Vận hành & Lãnh đạo (Admin/Back-office Portal): Điều phối, giám sát chuyến đi, quản lý hồ sơ, cấu hình phân quyền, báo cáo doanh thu và hiệu suất.
  - Tích hợp cổng thanh toán bên thứ ba (Payment Gateway) và dịch vụ thông báo (Notification Service).

---

## 2. CÁC BÊN LIÊN QUAN & TÁC NHÂN (STAKEHOLDERS & ACTORS)

| Tác nhân / Stakeholder | Loại | Mô tả vai trò |
| :--- | :--- | :--- |
| **Khách hàng (Customer)** | User Actor | Người tạo yêu cầu chuyến đi, thanh toán cước và đánh giá chất lượng dịch vụ. |
| **Tài xế (Driver)** | User Actor | Người tiếp nhận cuốc xe, thực hiện vận chuyển và cập nhật trạng thái cuốc xe theo thời gian thực. |
| **Nhân viên Vận hành (Operator)** | User Actor | Nhân sự trực tiếp theo dõi, hỗ trợ xử lý sự cố cuốc xe và quản lý tài xế/khách hàng. |
| **Quản trị hệ thống (System Admin)** | User Actor | Cấu hình tham số hệ thống, phân quyền tài khoản nội bộ và kiểm tra audit log. |
| **Ban Giám đốc (Management/Sponsor)** | Stakeholder | Tiếp nhận các báo cáo KPI, doanh thu, tỷ lệ hoàn thành chuyến; phê duyệt chính sách vận hành. |
| **Cổng Thanh toán (Payment Gateway)** | External System | Hệ thống xử lý giao dịch điện tử (không lưu thẻ nhạy cảm trên CAB System). |
| **Dịch vụ Thông báo (Notification Engine)** | External System | Hệ thống phát thông báo qua Push Notification/SMS/Email. |

---

## 3. YÊU CẦU CHỨC NĂNG (FUNCTIONAL REQUIREMENTS - FR)

### 3.1 Module Quản lý Tài khoản & Xác thực (Account & Authentication)
- **FR-AUTH-01:** Khách hàng và Tài xế có thể đăng ký tài khoản mới và đăng nhập thông qua cơ chế xác thực an toàn (OTP SMS/Mật khẩu).
- **FR-AUTH-02:** Hệ thống cho phép Nhân viên vận hành tạo và cấp tài khoản làm việc cho Tài xế trực tiếp trên Admin Portal.
- **FR-AUTH-03:** Người dùng có thể cập nhật thông tin hồ sơ cá nhân; Tài xế có thể cập nhật thông tin phương tiện (biển số, dòng xe, loại dịch vụ).
- **FR-AUTH-04:** Hệ thống hỗ trợ Phân quyền dựa trên vai trò (RBAC) cho nhân viên nội bộ, hạn chế nhân viên thông thường truy cập dữ liệu/hành động nhạy cảm.

### 3.2 Module Đặt xe & Phân bổ Chuyến đi (Ride Booking & Dispatching)
- **FR-BOOK-01:** Khách hàng nhập điểm đón, điểm đến, lựa chọn loại dịch vụ/loại xe và gửi yêu cầu đặt xe.
- **FR-BOOK-02:** Hệ thống hiển thị trạng thái tìm tài xế, thông tin tài xế nhận cuốc, định vị phương tiện và thời gian dự kiến đến (ETA).
- **FR-DISP-01:** Thuật toán phân bổ tự động quét và lựa chọn tài xế khả dụng gần điểm đón nhất dựa trên toạ độ GPS và trạng thái trực tuyến.
- **FR-DISP-02:** Khi có cuốc xe được điều phối, tài xế nhận được thông tin mời cuốc và có quyền "Chấp nhận" hoặc "Từ chối" trong một khoảng thời gian giới hạn (Timeout).
- **FR-DISP-03:** Nếu tài xế từ chối hoặc quá thời gian phản hồi, hệ thống phải tự động chuyển cuốc đến tài xế phù hợp tiếp theo mà không yêu cầu khách hàng tạo lại đơn.
- **FR-DISP-04:** Trường hợp quét hết danh sách tài xế mà không có người tiếp nhận, hệ thống phải thông báo rõ ràng lý do cho khách hàng.

### 3.3 Module Thực thi Chuyến đi & Định vị (Ride Execution & Tracking)
- **FR-RIDE-01:** Tài xế có thể chuyển đổi trạng thái làm việc: `Trực tuyến / Sẵn sàng (Online)` hoặc `Ngoại tuyến (Offline)`.
- **FR-RIDE-02:** Ứng dụng tài xế gửi toạ độ GPS định kỳ về máy chủ để cập nhật vị trí thời gian thực.
- **FR-RIDE-03:** Tài xế cập nhật tuần tự trạng thái chuyến đi:
  `Đã nhận cuốc` -> `Đã đến điểm đón` -> `Đã đón khách / Bắt đầu di chuyển` -> `Hoàn thành chuyến`.
- **FR-RIDE-04:** Khách hàng và Bộ phận vận hành có thể xem vị trí và trạng thái chuyến đi theo thời gian thực trên bản đồ.

### 3.4 Module Tính cước, Thanh toán & Đánh giá (Fare, Payment & Rating)
- **FR-PAY-01:** Hệ thống tự động tính toán tổng số tiền cước sau khi hoàn tất chuyến dựa trên loại xe, quãng đường và thời gian di chuyển.
- **FR-PAY-02:** Hỗ trợ 2 phương thức thanh toán: Tiền mặt (Cash) và Thanh toán điện tử (E-Payment qua bên thứ ba).
- **FR-PAY-03:** Hệ thống không lưu trữ thông tin thẻ ngân hàng/CVV trực tiếp mà ủy quyền xử lý an toàn cho Payment Gateway bên ngoài.
- **FR-PAY-04:** Trường hợp thanh toán điện tử thất bại, hệ thống gửi cảnh báo lỗi cho khách hàng và cho phép thử lại hoặc đổi phương thức thanh toán.
- **FR-RATE-01:** Khách hàng xem được lịch sử cuốc xe chi tiết kèm biên lai thanh toán.
- **FR-RATE-02:** Khách hàng có thể chấm điểm (1 - 5 sao) và gửi đánh giá nhận xét về tài xế sau khi hoàn thành chuyến đi.

### 3.5 Module Thông báo (Notification Engine)
- **FR-NOTI-01:** Gửi thông báo đến Khách hàng tại các mốc: Tiếp nhận yêu cầu, Có tài xế nhận chuyến, Tài xế đã đến điểm đón, Hoàn thành chuyến và Kết quả thanh toán.
- **FR-NOTI-02:** Gửi thông báo tức thì đến Tài xế khi có yêu cầu cuốc xe mới hoặc khi khách hàng thao tác hủy chuyến.
- **FR-NOTI-03:** Kiến trúc xử lý thông báo được thiết kế dạng module hóa, sẵn sàng mở rộng thêm kênh gửi (Push, SMS, ZNS) mà không làm gián đoạn hệ sinh thái.

### 3.6 Module Quản trị & Báo cáo (Admin Portal & Reporting)
- **FR-ADM-01:** Giao diện tập trung hiển thị danh sách tất cả các chuyến đi đang diễn ra (Live Trips) và trạng thái của toàn bộ tài xế trên bản đồ/danh sách.
- **FR-ADM-02:** Nhân viên vận hành có công cụ can thiệp, tra cứu lịch sử giao dịch và hỗ trợ giải quyết các cuốc xe phát sinh sự cố hoặc khiếu nại.
- **FR-ADM-03:** Bảng điều khiển phân tích số liệu (Dashboard) dành cho Lãnh đạo: Tổng số cuốc xe, Doanh thu, Tỷ lệ cuốc hoàn thành, Tỷ lệ hủy và Báo cáo hiệu suất từng tài xế.
...
- **NFR-PERF-01:** Hệ thống đảm bảo thời gian phản hồi cho các thao tác tìm và điều phối chuyến dưới 3 giây vào giờ cao điểm.
- **NFR-PERF-02:** Các thành phần cốt lõi (Core Booking, Payment, Notification) được thiết kế tách rời (Decoupled / Microservices-ready), có khả năng mở rộng độc lập (Horizontal Scaling) khi lưu lượng tăng đột biến.
- **NFR-PERF-03:** Lỗi phát sinh tại module Thanh toán hoặc Thông báo tuyệt đối không được gây sập hay gián đoạn module Đặt xe chính.

### 4.2 Tính sẵn sàng & Độ tin cậy (Availability & Reliability)
- **NFR-AVAIL-01:** Độ sẵn sàng của hệ thống đạt tối thiểu 99.5% trong khung giờ hoạt động bình thường.
- **NFR-RELI-01:** Hệ thống cho phép triển khai nâng cấp từng phần (CI/CD, Rolling Updates) nhằm hạn chế tối đa thời gian chết (Downtime).

### 4.3 Bảo mật & An toàn dữ liệu (Security & Compliance)
- **NFR-SEC-01:** Bắt buộc xác thực và cấp quyền cho tất cả các yêu cầu API từ mobile app và web admin (sử dụng Token OAuth2/JWT).
- **NFR-SEC-02:** Mã hóa dữ liệu đường truyền bằng TLS 1.3 và mã hóa dữ liệu nhạy cảm ở trạng thái lưu trữ (Data at Rest).
- **NFR-SEC-03:** Tuân thủ tiêu chuẩn bảo mật thanh toán; tuyệt đối không ghi nhận/lưu trữ dữ liệu thẻ tín dụng trên máy chủ của CAB System.
- **NFR-SEC-04:** Ghi lại Audit Log toàn bộ thao tác nhạy cảm của tài khoản quản trị (sửa thông tin tài khoản, hủy chuyến thủ công, hoàn tiền) để phục vụ tra soát sự cố.

---

## 5. CÁC QUY TẮC NGHIỆP VỤ & ĐIỂM CẦN LÀM RÕ (BUSINESS RULES & OPEN ISSUES)

### 5.1 Các quy tắc nghiệp vụ dự kiến (Tentative Business Rules)
- **BR-01 (Thời gian chờ tài xế):** Tài xế có khoảng thời gian cố định (dự kiến: 15-30 giây) để phản hồi nhận chuyến trước khi hệ thống chuyển sang tài xế kế tiếp.
- **BR-02 (Bán kính tìm kiếm):** Bán kính quét tài xế ban đầu là R km; nếu không tìm thấy, hệ thống có thể tự động mở rộng theo từng nấc nhưng không vượt quá R_max.

### 5.2 Các câu hỏi mở cần BA làm rõ với Khách hàng (Open Issues for Clarification)
1. **Công thức tính cước:** Có áp dụng giá cước linh hoạt giờ cao điểm (Surge Pricing) không, hay áp dụng biểu phí cố định theo km + thời gian?
2. **Chính sách hủy chuyến:** Điều kiện để Khách hàng hoặc Tài xế hủy chuyến miễn phí là gì? Sau bao lâu kể từ lúc đặt thì bắt đầu tính phí phạt hủy chuyến?
3. **Xử lý sự cố kết nối:** Khi ứng dụng của tài xế bị mất kết nối mạng giữa chuyến đi, cơ chế tính toán lại quãng đường di chuyển thực tế (dựa trên GPS cache hay lộ trình dự kiến) sẽ hoạt động như thế nào?
4. **Thời gian lưu trữ dữ liệu (Data Retention):** Dữ liệu định vị chi tiết (GPS breadcrumbs) của tài xế và lịch sử cuốc xe cần lưu trữ nóng trong bao lâu trước khi chuyển sang dạng lưu trữ nén (Cold Storage)?
